...
public class RewardTagUtils {
    ...
    public static boolean shouldRender(PlayerEntity player) {
        if (player == MinecraftClient.getInstance().player && !The5zigMod.getConfig().getBool("showOwnNameTag"))
            return false;
        if (player.isInvisibleTo(MinecraftClient.getInstance().player))
            return false;
        if (player.hasPlayerRider())
            return false;
        int renderDist = 4096; // 64^2
        if (player.squaredDistanceTo(MinecraftClient.getInstance().player) > renderDist)
            return false;
        return shouldRenderTeam(player);
    }

    public static void render(MatrixStack stack, PlayerEntityRenderer renderer, String str, PlayerEntity pl,
