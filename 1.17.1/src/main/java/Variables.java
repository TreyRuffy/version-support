...
public class Variables implements IVariables, GLFWKeyCallbackI {
	...
	static {
		try {
			if (Transformer.REFLECTION == null) {
				...
				try {
					Transformer.REFLECTION = (ReflectionNames) Class.forName("eu.the5zig.mod.asm." + reflName)
							.newInstance();
	...
	private static final List<PotionEffectImpl> DUMMY_POTIONS = Arrays.asList(
			new PotionEffectImpl("potion.jump", 20, "0:01", 1, 10, true, true, 0x22ff4c),
	...
	public Variables() {
		...
		Mouse.init(new Mouse.MouseHandler() {
			...
			public boolean isButtonDown(int button) {
				return GLFW.glfwGetMouseButton(scaledResolution.getHandle(), button) == GLFW.GLFW_PRESS;
			}
		...
		Display.init(new Display.DisplayHandler() {
			...
			public boolean isActive() {
				return GLFW.glfwGetWindowAttrib(scaledResolution.getHandle(), GLFW.GLFW_FOCUSED) != 0;
			}
		});
	...
	public List<String> splitStringToWidth(String string, int width) {
		...
		return getFontrenderer().wrapLines(ChatComponentBuilder.fromLegacyText(string), width).stream()
				.map(ChatUtils::getText).collect(Collectors.toList());
	}
	...
	@Override
	public <E extends Row> IGuiList<E> createGuiList(Clickable<E> clickable, int width, int height, int top, int bottom,
			int left, int right, GuiArrayList<E> rows) {
	...
	@Override
	public <E extends Row> IGuiList<E> createGuiListChat(int width, int height, int top, int bottom, int left,
			int right, int scrollx, List<E> rows, GuiListChatCallback callback) {
		return new GuiListChat<E>(width, height, top, bottom, left, right, scrollx, rows, callback);
	}

	@Override
	public IFileSelector createFileSelector(File currentDir, int width, int height, int left, int right, int top,
			int bottom, Callback<File> callback) {
	...
	@Override
	public IColorSelector createColorSelector(int id, int x, int y, int width, int height, String label,
	...
	public void playSound(String domain, String sound, float pitch) {
		getMinecraft().getSoundManager()
				.play(PositionedSoundInstance.master(new SoundEvent(new ResourceLocation(domain, sound)), pitch));
	}
	...
	public boolean isPlayerListShown() {
		return (getGameSettings().keyPlayerList.isPressed())
				&& ((!getMinecraft().isIntegratedServerRunning()) || (getServerPlayers().size() > 1));
	}
	...
	public void joinServer(String host, int port) {
		...
		displayScreen(new ConnectScreen((Screen) getMinecraftScreen(), getMinecraft(),
				new ServerInfo(host, host + ":" + port, false)));
	}
	...
	public String getBiome() {
		...
		return localObject.getBiomeArray()
				.getBiomeForNoiseGen((int) getPlayerPosX(), (int) getPlayerPosY(), (int) getPlayerPosZ()).getCategory()
				.getName();
	}
	...
	public String getEntityCount() {
		if (getMinecraft().worldRenderer == null || getMinecraft().world == null)
			return "No world";
	...
	public int getFoodLevel() {
		return isSpectatingSelf() ? ((PlayerEntity) getSpectatingEntity()).getHungerManager().getFoodLevel()
				: getPlayer().getHungerManager().getFoodLevel();
	}
	...
	public float getResistanceFactor() {
		...
		if (referenceDamage <= 0.0F) {
			...
			int enchantmentModifierDamage = 0; // ZIG116 EnchantmentHelper.getAttackDamage(getPlayer().inventory.armor,
												// EntityGroup.DEFAULT);
	...
	private PotionEffectImpl wrapPotionEffect(StatusEffectInstance potionEffect) {
		...
		return new PotionEffectImpl(potion == null ? "" : potionEffect.getTranslationKey(), potionEffect.getDuration(),
				StatusEffectUtil.durationToString(potionEffect, 1), potionEffect.getAmplifier() + 1,
				potion == null ? -1 : StatusEffect.getRawId(potion),
				potion == null || potionEffect.getEffectType().isBeneficial(), true,
				potion == null ? 0 : potionEffect.getEffectType().getColor());
	}
	...
	public ItemStack getItemInOffHand() {
		return getPlayerItemInOffHand() == null ? null : new WrappedItemStack(getPlayerItemInOffHand());
	}
	...
	public net.minecraft.item.ItemStack getPlayerItemInOffHand() {
		return getPlayer().getOffHandStack();
	}

	public net.minecraft.item.ItemStack getArmorItemBySlot(int slot) {
		return getPlayer().getInventory().getArmorStack(slot);
	}
	...
	public int getItemCount(String key) {
		int count = 0;
		for (net.minecraft.item.ItemStack itemStack : getPlayer().getInventory().main) {
			if (itemStack == null)
				continue;
	...
	public void setSelectedHotbarSlot(int slot) {
		getPlayer().getInventory().selectedSlot = slot;
		getNetworkManager().send(new UpdateSelectedSlotS2CPacket(slot));
	}
	...
	public void renderItem(net.minecraft.item.ItemStack itemStack, int x, int y) {
		...
		itemRenderer.renderInGuiWithOverrides(getPlayer(), itemStack, x, y, 0);
		if (cooldown > 0.0f) {
	...
	public String getKeyDisplayStringShort(int key) {
		String tempName;
		return key < 0 ? "M" + (key + 101)
				: (key < 256
						? ((tempName = GLFW.glfwGetKeyName(key, -1)) == null ? GLFW.glfwGetKeyName(-1, key) : tempName)
						: String.format("%c", (char) (key - 256)).toUpperCase());
	}
	...
	public void messagePlayer(Text chatComponent) {
		boolean cancel = MinecraftFactory.getClassProxyCallback().shouldCancelChatMessage(
				chatComponent.getString().replace(ChatColor.RESET.toString(), ""), chatComponent);
		if (!cancel) {
	...
	public void sendCustomPayload(String channel, ByteBuf payload) {
		if (getNetworkManager() != null) {
			getNetworkManager()
					.send(new CustomPayloadC2SPacket(Identifier.splitOn(channel, ':'), new PacketByteBuf(payload)));
		}
	}
	...
	public void fillDynamicImage(Object dynamicImage, BufferedImage image) {
		image.getRGB(0, 0, image.getWidth(), image.getHeight(),
				((NativeImageBackedTexture) dynamicImage).getImage().makePixelArray(), 0, image.getWidth());
	...
	public void renderTextureOverlay(int x1, int x2, int y1, int y2) {
		...
		var5.begin(DrawMode.LINE_STRIP, VertexFormats.POSITION_TEXTURE_COLOR);
		var5.vertex(x1, y2, 0.0D).texture(0.0F, ((float) y2 / 32.0F)).color(64, 64, 64, 255).next();
		var5.vertex(x1 + x2, y2, 0.0D).texture(((float) x2 / 32.0F), ((float) y2 / 32.0F)).color(64, 64, 64, 255)
				.next();
		var5.vertex(x1 + x2, y1, 0.0D).texture(((float) x2 / 32.0F), ((float) y1 / 32.0F)).color(64, 64, 64, 255)
				.next();
	...
	public void dispatchKeypresses() {
		/*
		 * ZIG116 int eventKey = org.lwjgl.input.Keyboard.getEventKey(); int currentcode
		 * = eventKey == 0 ? org.lwjgl.input.Keyboard.getEventCharacter() : eventKey; if
		 * ((currentcode == 0) || (org.lwjgl.input.Keyboard.isRepeatEvent())) return;
		 * 
		 * if (org.lwjgl.input.Keyboard.getEventKeyState()) { int keyCode = currentcode
		 * + (eventKey == 0 ? 256 : 0);
	...
	public void sendChatComponent(MessageComponent component, boolean secondChat) {
		...
		if (secondChat)
			gui2ndChat.printChatMessage(result);
		else
			getMinecraft().inGameHud.getChatHud().addMessage(result);
	}
}
